...
#y_tests=[]
#for i in range(5):
#    filename = join('../data/folds/', 'fold')
#    logging.info('loading fold {} coef to dir ({})'.format(i, filename))
...
#    x_trains.append(filename_trainx)
#    y_trains.append(filename_trainy)
#    x_tests.append(filename_testx)
#    y_tests.append(filename_testy)
...
def run_lr(train_X, test_X, train_y, test_y, c=1.0,solver='lbfgs', penalty='l2', seed=0):
    clf = LogisticRegression(C=c,random_state=seed,solver=solver,penalty=penalty)
    clf.fit(train_X, train_y)
    ypred = np.array([i[1] for i in clf.predict_proba(test_X)])
    ypred_binary = clf.predict(test_X)
    metrics = gen_eval_metrics(test_y, ypred, svm_binary=ypred_binary)
...
for fi in range(len(input_files)):
    ...
    para_c = [0.25,0.5,0.75,1.0,1.25,1.5,1.75,2.0]
    kernel_set = ['linear','poly']
    #res = []
    for kernel in kernel_set:
        for c in para_c:
            ...
            for i in range(num_folds):
                ...
                metrics = run_svm(train_X, test_X, train_y, test_y,c=c,kern=kernel)
    ...
    for p in penaltys:
        for s in solvers:
            for c in para_c:
                if (s=='newton-cg' or s=='lbfgs' or s=='sag') and p!='l2':
                    continue
                ...
                for met in fold_metrics:
                    ...
                    auc.append(met[4])
                    auprc.append(met[5])
                res_lr.append({'label':input_labels[fi],'c':c,'solvers':s,'penalty':p,'Accuracy':np.average(acc),'Accuracy_sd':math.sqrt(np.var(acc)),'Precision':np.average(pre),'Precision_sd':math.sqrt(np.var(pre)),'Recall':np.average(rec),'Recall_sd':math.sqrt(np.var(rec)),'F1Score':np.average(f1),'F1Score_sd':math.sqrt(np.var(f1)),'AUC':np.average(auc),'AUC_sd':math.sqrt(np.var(auc)),'AUPRC':np.average(auprc),'AUPRC_sd':math.sqrt(np.var(auprc))})
    depth_arr = [2,6,10]
    est_arr = [10,50,100]
    cri_arr = ['entropy','gini']
    #res_rf = []
        ...
        for est in est_arr:
            for cri in cri_arr:
                ...
                for i in range(num_folds):
                    ...
                    test_X=test_X.to_numpy()
                    test_y=test_y.to_numpy()
                    metrics = run_rf(train_X, test_X, train_y, test_y,depth=depth,est=est,c=cri)
                    fold_metrics.append(metrics)
                acc,pre,rec,f1,auc,auprc = [],[],[],[],[],[]
                for met in fold_metrics:
                    acc.append(met[0])
                    ...
                    auc.append(met[4])
                    auprc.append(met[5])
                res_rf.append({'label':input_labels[fi],'depth':depth,'estimators':est,'criterion':cri,'Accuracy':np.average(acc),'Accuracy_sd':math.sqrt(np.var(acc)),'Precision':np.average(pre),'Precision_sd':math.sqrt(np.var(pre)),'Recall':np.average(rec),'Recall_sd':math.sqrt(np.var(rec)),'F1Score':np.average(f1),'F1Score_sd':math.sqrt(np.var(f1)),'AUC':np.average(auc),'AUC_sd':math.sqrt(np.var(auc)),'AUPRC':np.average(auprc),'AUPRC_sd':math.sqrt(np.var(auprc))})
...
res_lrdf = pd.DataFrame(res_lr)
res_rfdf = pd.DataFrame(res_rf)
res_df.to_csv("svm.csv",index=False)
res_lrdf.to_csv("lr.csv",index=False)
res_rfdf.to_csv("rf.csv",index=False)
