...
class DataModelLoader():
    ...
    def load_model(self, model_dir_, model_name):
        ...
        fs_model = model_factory.get_model(params['models'])
        ##for cross calidation, select best auc fold
        perf = params['scores']
	...
	for i in range(len(auc)):
        ...
        weights_file = join(model_dir_, 'fs/{}_{}.h5'.format(model_name,maxind))
        model = fs_model.load_model(weights_file,by_name=True,skip_mismatch=True)
        return model
